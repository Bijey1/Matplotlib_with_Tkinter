...
fig, ax = plt.subplots(figsize=(9, 8),facecolor="#F3F3F3")  # width=8 inches, height=6 inches
...
def createBarPlot(xValues,yValues,colorBar,edgeColor,hatchBar,widthBar,first,toggle,labels):
    ...
    if (first):  # If first time or cleared
        ...
        if not checkHori.get():
            ...
            bars = ax.barh(finalValues["x"], finalValues["y"], height=finalValues["widthBar"],
                          color=finalValues["colorBar"], edgecolor=finalValues["edgeColor"],
                          hatch=finalValues["hatchBar"])
...
colorOption=[  "red", "blue", "yellow", "green", "purple", "orange", "pink", "brown",
    "cyan", "magenta", "lime", "teal", "violet", "indigo", "maroon",
    "navy", "olive", "gold", "silver", "turquoise",
    "beige", "lavender", "tan", "skyblue",
    "khaki", "orchid", "tomato", "crimson", "darkgreen", "darkblue",
    "darkred", "darkorange", "lightblue", "lightgreen", "lightpink"]
...
#Color for Bar
hatchPatternVal=[] #This the main list
colorBar=[]
edgeColor=[]
...
linePlot=["Marker Shape","Marker Size","MFC","MEC","Line Style","Line Color","Line Width"]
...
def saveFormat(feature):
    ...
    #for val in entryValues:#Check if all entries are not empty
    #    if val.get().strip() == "":
    #        mesBox("Some fields are empty. Please complete all entries.")
    #       return
    #   
    ...
    for index, value in enumerate(entryValues): # Getting the data of values in entries
        ...
        if (val_text == ""): #new 
             #Check if chart label entries are empty
             mesBox("Some fields are empty. Please complete all entries.")
             return
        try:
            if(index >= 5):
            ...
            mesBox("Please enter numeric values in the Values box")
            print(f"The error is {value.get()}")
            return

    #Final Confirmation
    if(not feature):
     if(not confirmPop("Are you Sure?")):
         return
     mesBox("Successful")
   ...
   # Apply defaults only if empty
    ...
    if graphType == "Line Graph":#Plottig the Graph
        #The Format Values are: [shape= 'o', size= '6', mfc='red', mec='red', linestyle= '-', lineCol='red', lineWidth='1'] EXAMPLE
        plotGraph("line",finalChartLabelVal,x,y,formatValues,[],[],[],[],[],[],False,feature)
    elif graphType == "Bar Graph":#Plottig the Graph
        #The Format Values are: [shape= 'o', size= '6', mfc='red', mec='red', linestyle= '-', lineCol='red', lineWidth='1'] EXAMPLE
        plotGraph("bar",finalChartLabelVal,x,y,formatValues[0],colorBar,edgeColor,hatchPatternVal,[],[],[],False,feature)
    elif graphType == "Pie Graph":
        plotGraph("pie",finalChartLabelVal ,x, y, [], colorBar, edgeColor, pieHatch,pieColor,pieHatch,explodeVal,percent,feature)
...
leftSideHeadLabel = tk.Label(
    leftSideHeadFrame,
    text="Sales Data Input",
    fg="white",
    bg="#3E9063",
    font=("Arial", 12, "bold")
)
leftSideHeadLabel.pack(pady=5)
type =["Label","Value"]
...
def createChartLabel():
    ...
    for i in range(1):
        ...
        for j in range(3):
            ...
            # Label
            itemLabel = tk.Label(titleFrame, text=f"{titleLab[j]}:", bg="white", font=('Arial', 10))
        ...
        # Make all columns expandable
        for c in range(3):  # 2 columns * 2 (label+entry)
...
for i in range(2):
    ...
    for j in range(5):
        ...
        # Label
        itemLabel = tk.Label(leftLabelFrame, text=f"{type[i]} #{j + 1}:", bg="white", font=('Arial', 10))
        itemLabel.grid(row=count, column=col * 2, sticky="w", padx=5, pady=5)

        # Entry

        itemEntry = tk.Entry(leftLabelFrame, bg="white", highlightthickness=1)
...
#Frame for inputting Title, XLabel, and YLabel
...
submitButton = tk.Button(
    containerB, text="Submit", bg="#2E8B57",
    font=("Arial", 10, "bold"), fg="white",
    width=10, height=2,  command=lambda :saveFormat(False),
    activebackground="#3CB371"
)
submitButton.pack(side="left", padx=12)

resetButton = tk.Button(
    containerB, text="Reset", bg="#CD5C5C",
    font=("Arial", 10, "bold"), fg="white",
    width=10, height=2, command=resetButton,
...
# Right Header
rightSideHeadFrame = tk.Frame(rightSideFrame, bg="#3E9063", height=30)
rightSideHeadFrame.pack_propagate(False)


rightSideHeadLabel = tk.Label(
    rightSideHeadFrame,
    text="Sales Performance Figures",
    fg="white", bg="#3E9063",
    font=("Arial", 12, "bold")
)
rightSideHeadLabel.pack(pady=3)

# Checkbox panel (aligned, better spacing)
...
def createCheckBox(graphType):
    ...
    tk.Label(
        ...
        font=("Arial", 11, "bold"),
        bg="white", fg="#3E9063"
    ).grid(row=0, column=0, pady=(10, 10), sticky="w")

    if(graphType=="Line Graph"):

        tk.Checkbutton(
            checkBoxFrame, text="Toggle Grid", command=lambda: toggleFeature("grid"), variable=checkGrid,
            fg="black", bg="white", font=("Arial", 10)
        ).grid(row=1, column=0, pady=8, sticky="w")

        tk.Checkbutton(
            checkBoxFrame, text="Toggle Y Ticks", command=lambda: toggleFeature("yTick"), variable=checkyTick,
            fg="black", bg="white", font=("Arial", 10)
        ).grid(row=2, column=0, pady=8, sticky="w")

    elif graphType == "Bar Graph":
        print("POMASOK HERE ", graphType)
        tk.Checkbutton(
            checkBoxFrame, text="Toggle Grid", command=lambda: toggleFeature("grid"), variable=checkGrid,
            fg="black", bg="white", font=("Arial", 10)
        ).grid(row=1, column=0, pady=8, sticky="w")
        ...
        tk.Checkbutton(
            checkBoxFrame, text="Toggle Y Value Text", command=lambda: toggleFeature("textOnTop"), variable=checkTextOnTop,
            fg="black", bg="white", font=("Ari"
                                          "al", 10)
        ).grid(row=3, column=0, pady=8, sticky="w")

        tk.Checkbutton(
            checkBoxFrame, text="Toggle Legend", command=lambda: toggleFeature("legend"), variable=checkLegend,
            fg="black", bg="white", font=("Arial", 10)
        ).grid(row=4, column=0, pady=8, sticky="w")

        tk.Checkbutton(
            checkBoxFrame, text="Toggle Horizontal Bar", command=lambda: toggleFeature("shadow"), variable=checkHori,
            fg="black", bg="white", font=("Arial", 10)
        ...
        tk.Checkbutton(
            checkBoxFrame, text="Toggle Shadow", command=lambda: toggleFeature("shadow"), variable=checkShadow,
            fg="black", bg="white", font=("Arial", 10)
        ).grid(row=2, column=0, pady=8, sticky="w")

        tk.Checkbutton(
            checkBoxFrame, text="Toggle Percentages", command=lambda: toggleFeature("shadow"), variable=checkPercent,
            fg="black", bg="white", font=("Arial", 10)
        ).grid(row=3, column=0, pady=8, sticky="w")
